...
# TorchScript kernels for gate application
@torch.jit.script
def _apply_H_script(W_flat: torch.Tensor,
                    ...
                    x_idx: int,
                    z_idx: int) -> None:
    x_cols = W_flat[active_indices, :, x_idx].clone()
    z_cols = W_flat[active_indices, :, z_idx].clone()
    has_y = x_cols & z_cols

    W_flat[active_indices, :, x_idx] = z_cols
    W_flat[active_indices, :, z_idx] = x_cols

    x_rows_inv = W_inv_flat[active_indices, x_idx, :].clone()
    z_rows_inv = W_inv_flat[active_indices, z_idx, :].clone()
    W_inv_flat[active_indices, x_idx, :] = z_rows_inv
    W_inv_flat[active_indices, z_idx, :] = x_rows_inv

    if torch.any(has_y):
        phase_updates = has_y.to(torch.int8) * 2
        phase_flat[active_indices] ^= phase_updates
...
def _apply_S_script(W_flat: torch.Tensor,
                    ...
                    z_idx: int) -> None:
    x_cols = W_flat[active_indices, :, x_idx].clone()
    z_cols = W_flat[active_indices, :, z_idx]
    has_x = x_cols

    W_flat[active_indices, :, x_idx] = x_cols ^ z_cols

    x_rows_inv = W_inv_flat[active_indices, x_idx, :]
    z_rows_inv = W_inv_flat[active_indices, z_idx, :]
    W_inv_flat[active_indices, z_idx, :] = z_rows_inv ^ x_rows_inv

    if torch.any(has_x):
        phase_updates = has_x.to(torch.int8)
        phase_flat[active_indices] = (phase_flat[active_indices] + phase_updates) % 4
...
def _apply_CNOT_script(W_flat: torch.Tensor,
                       ...
                       cz: int,
                       tz: int) -> None:
    W_flat[active_indices, tx, :] ^= W_flat[active_indices, cx, :]
    W_flat[active_indices, cz, :] ^= W_flat[active_indices, tz, :]

    cx_rows_inv = W_inv_flat[active_indices, :, cx]
    tx_rows_inv = W_inv_flat[active_indices, :, tx]
    W_inv_flat[active_indices, :, cx] ^= tx_rows_inv

    tz_rows_inv = W_inv_flat[active_indices, :, tz]
...
class CliffordMap:
    ...
    def _pauli_string_to_symplectic_vectorized(self, pauli_strings: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i, pauli_string in enumerate(pauli_strings):
            ...
            vec = torch.zeros(self.N2, dtype=torch.bool, device=self.device)
    ...
    def transform_paulis(self, pauli_vecs: torch.Tensor, chunk_size: int = 4096) -> torch.Tensor:
        """Transform Pauli vectors under all Clifford maps using maintained W_inv.
        ...
        # NEW: Use maintained W_inv instead of computing inverse
        for start in range(0, n_tableaus, chunk_size):
            end = min(start + chunk_size, n_tableaus)
            
            # Directly use pre-computed W_inv (int8)
            W_inv_chunk = self._W_inv_flat[start:end]

            # Multiply in GF(2) -> (chunk, 2n, n_paulis)
            p_out = GF2Ops.matmul(W_inv_chunk, pauli_vecs_t, validate=False)
    ...
    def validate_inverse(self, tolerance: float = 1e-6) -> bool:
        """Validate that W_inv is truly the inverse of W for all active tableaus"""
        active_indices = self._active_flat.nonzero(as_tuple=True)[0]
        ...
        for idx in sample_indices:
            ...
            # Compute W @ W_inv in GF(2)
            identity_check = GF2Ops.matmul(W.unsqueeze(0), W_inv.unsqueeze(0), validate=False).squeeze(0)
            
            # Should be identity
            expected_identity = torch.eye(self.N2, dtype=torch.bool, device=self.device)
            
            if not torch.equal(identity_check, expected_identity):
