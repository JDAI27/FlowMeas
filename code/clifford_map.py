...
def _apply_H_script(W_flat: torch.Tensor,
                    ...
                    qubit: int,
                    n_qubits: int) -> None:
    """H gate: Swap X_a and Z_a bits (Aaronson-Gottesman algorithm)."""
    n = n_qubits
    x_bits = W_flat[active_indices, :, qubit].clone()
    z_bits = W_flat[active_indices, :, n + qubit]

    # H(Y) = -Y: phase flip by 2 for Y components
    has_y = x_bits & z_bits
    phase_inc = (has_y.to(torch.int8) * 2)
    ...
    # Update W_inv: swap corresponding rows
    x_rows_inv = W_inv_flat[active_indices, qubit, :].clone()
    z_rows_inv = W_inv_flat[active_indices, n + qubit, :]
...
def _apply_S_script(W_flat: torch.Tensor,
                    ...
                    qubit: int,
                    n_qubits: int) -> None:
    """S gate: z ← z ⊕ x with phase update (Aaronson-Gottesman algorithm)."""
    n = n_qubits
    x_bits = W_flat[active_indices, :, qubit]
    z_bits = W_flat[active_indices, :, n + qubit]

    # Phase increment: Δr = 2 * (x · z)
    phase_inc = (x_bits & z_bits).to(torch.int8) * 2
    phase_flat[active_indices] = (phase_flat[active_indices] + phase_inc) & 3

    W_flat[active_indices, :, n + qubit] ^= x_bits

    # Update W_inv: X_a row ← X_a ⊕ Z_a
    z_row_inv = W_inv_flat[active_indices, n + qubit, :]
    W_inv_flat[active_indices, qubit, :] ^= z_row_inv
...
def _apply_CNOT_script(W_flat: torch.Tensor,
                       ...
                       target: int,
                       n_qubits: int) -> None:
    """CNOT gate: X_c → X_c X_t, Z_t → Z_c Z_t (Aaronson-Gottesman algorithm)."""
    n = n_qubits
    cx = control
    tx = target
    cz = n + control
    ...
    cx_row_bits = W_flat[active_indices, :, cx]
    tx_row_bits = W_flat[active_indices, :, tx]
    cz_row_bits = W_flat[active_indices, :, cz]
    tz_row_bits = W_flat[active_indices, :, tz]

    # Phase increment: Δr = 2 * [x_c · z_t · (x_t ⊕ z_c ⊕ 1)]
    phase_inc = (cx_row_bits & tz_row_bits & (tx_row_bits ^ cz_row_bits ^ 1)).to(torch.int8) * 2
    phase_flat[active_indices] = (phase_flat[active_indices] + phase_inc) & 3

    W_flat[active_indices, :, tx] ^= cx_row_bits
    W_flat[active_indices, :, cz] ^= tz_row_bits

    # Update W_inv with row operations
    tx_rows_inv = W_inv_flat[active_indices, tx, :]
    cz_rows_inv = W_inv_flat[active_indices, cz, :]
    W_inv_flat[active_indices, cx, :] ^= tx_rows_inv
    W_inv_flat[active_indices, tz, :] ^= cz_rows_inv


class TensorPool:
    """Memory pool for reusing temporary tensors."""
...
class CliffordMap:
    ...
    def _pauli_commutation_phase(self, has_x1: bool, has_z1: bool, has_x2: bool, has_z2: bool) -> int:
        """Calculate phase from commuting two Paulis (mod 4)."""
        ...
        if has_x1 and not has_z1:
            if has_z2 and not has_x2:
                return 3
            elif is_y2:
                return 1
        elif has_z1 and not has_x1:
            if has_x2 and not has_z2:
                return 1
            elif is_y2:
                return 3
        elif is_y1:
    ...
    def apply_H(self, qubit: int, mask: Optional[torch.Tensor] = None):
        """Apply Hadamard gate: X → Z, Y → -Y, Z → X."""
        if mask is None:
            mask = self.active
        else:
    ...
    def apply_HS(self, qubit: int, mask: Optional[torch.Tensor] = None):
        """Apply HS gate (S then H): X → -Y, Y → -Z, Z → X."""
        if mask is None:
            mask = self.active
        else:
            mask = self._ensure_device(mask)
    ...
    def apply_SH(self, qubit: int, mask: Optional[torch.Tensor] = None):
        """Apply SH gate (H then S): X → Z, Y → X, Z → Y."""
        if mask is None:
            mask = self.active
        else:
            mask = self._ensure_device(mask)
    ...
    def apply_HSH(self, qubit: int, mask: Optional[torch.Tensor] = None):
        """Apply HSH gate (H then S then H)."""
        if mask is None:
            mask = self.active
        else:
            mask = self._ensure_device(mask)
    ...
    def apply_actions_step(self,
        ...
        """Apply a single layer of actions."""
        if mask is None:
            mask = self.active
        else:
            mask = self._ensure_device(mask)
    ...
    def apply_action(self,
                             ...
                             action_map: Dict[int, Tuple]):
        """Apply a set of circuits in parallel."""
    ...
    def _pauli_string_to_symplectic(self, pauli_strings: List[str]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for start_val in range(max_start + 1):
            ...
            pauli_parts = str_tensor[indices, start_val:start_val + self.n_qubits]
            ...
            y_counts = is_y.sum(dim=1).to(uncached_phases.dtype)
    ...
    def transform_paulis(self, pauli_vecs: torch.Tensor, chunk_size: int = 4096) -> torch.Tensor:
        """Transform Pauli vectors under all Clifford maps: U P U†."""
    ...
    def prob_P_single(self, pauli_string: str) -> torch.Tensor:
        """Compute probability for a single Pauli string."""
    ...
    def to_flat_tensors_active_only(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """Convert active tableaus to flat tensors (W matrix only)."""
        mat_size = self.N2 * self.N2
        feature_size = mat_size
    ...
    def forward_mlp_masked(self, mlp: torch.nn.Module) -> torch.Tensor:
        """Forward pass through MLP using Clifford map representation."""
        flat_active, indices = self.to_flat_tensors_active_only()
        
        if flat_active.shape[0] == 0:
            dummy_input = torch.zeros(1, self.N2 * self.N2, device=self.device)
            with torch.no_grad():
    ...
    def to_flat_tensors(self) -> Tuple[torch.Tensor, torch.Tensor]:
        """Convert W matrices to flat tensors."""
        mat_size = self.N2 * self.N2
    ...
    def _get_active_indices_flat(self, mask: torch.Tensor) -> Tuple[torch.Tensor, int]:
        """Convert 2D mask to flat indices."""
    ...
    def validate_inverse(self, tolerance: float = 1e-6) -> bool:
        """Validate that W_inv is the correct inverse of W by checking a sample."""
        active_indices = self._active_flat.nonzero(as_tuple=True)[0]
        ...
        for idx in sample_indices:
            ...
            lhs = GF2Ops.matmul(W_inv.unsqueeze(0), W.unsqueeze(0), validate=False).squeeze(0)
            rhs = GF2Ops.matmul(W.unsqueeze(0), W_inv.unsqueeze(0), validate=False).squeeze(0)
